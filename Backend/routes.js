...
router.get('/allusers',  checkAuthenticated, isAdmin, (req, res) => {
  getUsers()
    .then((results) => { res.type('json').send(JSON.stringify(results, null, 2)); })
    .catch((error) => { res.status(500).send('Hiba az adatok letöltése során! (Adatbázis hiba)' + (error)); });
});
...
router.get('/userapplied/:user_id', checkAuthenticated, function (req, res) {
...
router.get('/eventpass/:pass_code',  checkAuthenticated, isAdmin, async function (req, res) {
  try {
...
router.post("/newadmin",  checkAuthenticated, isAdmin, AddNewAdmin(connection));
router.post('/login',  login(connection));


// Új esemény létrehozása 
router.post("/newevent",  checkAuthenticated, isAdmin, async (req, res, next) => {
  try {
...
router.post('/applyToLocation', checkAuthenticated, async (req, res) => {
...
router.post('/cancelApplication',  checkAuthenticated, async (req, res) => {
  try {
...
// Jelszó frissítése
router.put('/refreshPassword',checkAuthenticated, async (req, res) => {
  try {
...
router.delete('/deleteEvent/:id',  checkAuthenticated, isAdmin, async (req, res) => {
...
router.delete("/deleteUser/:id", checkAuthenticated, async (req, res) => {
  const id = req.params.id;
  try {
